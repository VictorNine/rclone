...
const (
	privateKey = ""

	publicKey = "GCAYSAUBQEAMHGTPDR4XNF32KMZ6DUO65VFW42B72SJ6T4QC3YRELJJYZOTAYPEMXRYDTON2NZWZX7S6WNM3NLSTVTUJSZDHARV2K5G5AXFKTLNNOGL65GPA7U4VH272ZXYFSW2PGCB7UMIED5NVWI7VBQ6HIEVIBEHYBBXK3S23HBDHREADOIGDFDNJ5VXG4D53PJOC44YSBSVTLQK7VZEFAIBQCAAB"
)
...
func TestEncryptSegment(t *testing.T) {
	c, _ := newCipher(NameEncryptionStandard, "", "", true, privateKey, publicKey)
	for _, test := range []struct {
		in       string
		expected string
	}{
...
func TestEncryptFileName(t *testing.T) {
	...
	c, _ = newCipher(NameEncryptionStandard, "", "", false, privateKey, publicKey)
	assert.Equal(t, "p0e52nreeaj0a5ea7s64m4j72s", c.EncryptFileName("1"))
	...
	// Now off mode
	c, _ = newCipher(NameEncryptionOff, "", "", true, privateKey, publicKey)
	assert.Equal(t, "1/12/123.bin", c.EncryptFileName("1/12/123"))
	// Obfuscation mode
	c, _ = newCipher(NameEncryptionObfuscated, "", "", true, privateKey, publicKey)
	...
	c, _ = newCipher(NameEncryptionObfuscated, "", "", false, privateKey, publicKey)
...
func TestDecryptFileName(t *testing.T) {
	for _, test := range []struct {
		...
		c, _ := newCipher(test.mode, "", "", test.dirNameEncrypt, privateKey, publicKey)
...
func TestEncDecMatches(t *testing.T) {
	for _, test := range []struct {
		...
		c, _ := newCipher(test.mode, "", "", true, privateKey, publicKey)
...
func TestEncryptedSize(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{1, 160 + 16 + 1},
		{65536, 160 + 16 + 65536},
		{65537, 160 + 16 + 65536 + 16 + 1},
		{1 << 20, 160 + 16*(16+65536)},
		{(1 << 20) + 65535, 160 + 16*(16+65536) + 16 + 65535},
		{1 << 30, 160 + 16384*(16+65536)},
		{(1 << 40) + 1, 160 + 16777216*(16+65536) + 16 + 1},
	} {
...
func TestDecryptedSize(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{160 + 1, ErrorEncryptedFileBadHeader},
		{160 + 16, ErrorEncryptedFileBadHeader},
		{160 + 16 + 65536 + 1, ErrorEncryptedFileBadHeader},
		{160 + 16 + 65536 + 16, ErrorEncryptedFileBadHeader},
	} {
...
func testEncryptDecrypt(t *testing.T, bufSize int, copySize int64) {
	c, err := newCipher(NameEncryptionStandard, "", "", true, privateKey, publicKey)
	assert.NoError(t, err)
...
var (
	file0 = []byte{0x52, 0x43, 0x4c, 0x4f, 0x4e, 0x45, 0x0, 0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xf, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x76, 0x89, 0x31, 0x3a, 0x97, 0xa5, 0xc3, 0x2b, 0xa2, 0xa2, 0xbe, 0x52, 0x1a, 0x92, 0x58, 0x55, 0xdc, 0xd9, 0x57, 0xc7, 0xa6, 0x49, 0x4a, 0x5d, 0x18, 0x92, 0x56, 0xc7, 0x5b, 0xf6, 0x33, 0x1b, 0xed, 0x3d, 0x37, 0x48, 0x8b, 0x7b, 0xd2, 0x69, 0xf, 0xce, 0x66, 0x7b, 0xfa, 0x54, 0x9f, 0xd1, 0x4e, 0x2e, 0xa7, 0xee, 0x23,
		0x3d, 0x25, 0x4e, 0xe, 0xc4, 0x1b, 0x7f, 0xef, 0xcc, 0x99, 0xaf, 0xa8, 0x1f, 0x4c, 0x9d, 0x5e, 0x5f, 0xb1, 0xb1, 0x59, 0x29, 0x97, 0x72, 0xd, 0x3f, 0x9, 0x99, 0x9d, 0x51, 0x77, 0x94, 0xe1, 0xa1, 0xc5, 0xd, 0xae, 0xe9, 0x92, 0x29, 0xa6, 0xc8, 0x70, 0xf7, 0x84, 0xf6, 0xce, 0x45, 0x2, 0x37, 0x4b, 0x6f, 0xf8, 0xda, 0x60, 0x13, 0x5b, 0x20, 0xbe, 0xa6, 0xd1, 0x4c, 0x9c, 0x65, 0x4d, 0xa4, 0x10, 0xf8, 0x16, 0x57, 0x92, 0x80, 0xe5, 0x8d, 0xc, 0xf0}

 0xe5, 0x8d, 0xc, 0xf0, 0x15, 0x24, 0x6a, 0x2c, 0xa, 0x87, 0x31, 0xd6, 0x48, 0x99, 0x6, 0x60, 0xb0, 0x4f, 0x83, 0x94, 0x7b, 0xe1, 0xa7, 0x40, 0xde, 0x90, 0x16, 0x1d, 0xf6, 0xda, 0xef, 0xaf, 0x4e, 0xd1, 0x47, 0x29}
)
...
func TestNewEncrypterErrUnexpectedEOF(t *testing.T) {
	...
	assert.Equal(t, int64(160), n) // Magic bytes + Nonce + Encrypted sessionkey = 8 + 24 + 128
}
...
func TestDecrypterRead(t *testing.T) {
	...
	for i := range file16copy {
		...
		if i < fileMagicSize {
			...
			// You actually do expect crypto/rsa: decryption error
			// TODO: Include test for the actual error
			//assert.NoError(t, err)
			// _, err = ioutil.ReadAll(fh)
...
func TestKey(t *testing.T) {
	...
	assert.Equal(t, [32]byte{0x74, 0x55, 0xc7, 0x1a, 0xb1, 0x7c, 0x86, 0x5b, 0x84, 0x71, 0xf4, 0x7b, 0x79, 0xac, 0xb0, 0x7e, 0xb3, 0x1d, 0x56, 0x78, 0xb8, 0xc, 0x7e, 0x2e, 0xaf, 0x4f, 0xc8, 0x6, 0x6a, 0x9e, 0xe4, 0x68}, c.nameKey)
	assert.Equal(t, [16]byte{0x76, 0x5d, 0xa2, 0x7a, 0xb1, 0x5d, 0x77, 0xf9, 0x57, 0x96, 0x71, 0x1f, 0x7b, 0x93, 0xad, 0x63}, c.nameTweak)

	require.NoError(t, c.Key("Potato", ""))
	assert.Equal(t, [32]byte{0xae, 0xea, 0x6a, 0xd3, 0x47, 0xdf, 0x75, 0xb9, 0x63, 0xce, 0x12, 0xf5, 0x76, 0x23, 0xe9, 0x46, 0xd4, 0x2e, 0xd8, 0xbf, 0x3e, 0x92, 0x8b, 0x39, 0x24, 0x37, 0x94, 0x13, 0x3e, 0x5e, 0xf7, 0x5e}, c.nameKey)
	assert.Equal(t, [16]byte{0x54, 0xf7, 0x2, 0x6e, 0x8a, 0xfc, 0x56, 0xa, 0x86, 0x63, 0x6a, 0xab, 0x2c, 0x9c, 0x51, 0x62}, c.nameTweak)

	require.NoError(t, c.Key("potato", "sausage"))
	assert.Equal(t, [32]uint8{0x8e, 0x9b, 0x6b, 0x99, 0xf8, 0x69, 0x4, 0x67, 0xa0, 0x71, 0xf9, 0xcb, 0x92, 0xd0, 0xaa, 0x78, 0x7f, 0x8f, 0xf1, 0x78, 0xbe, 0xc9, 0x6f, 0x99, 0x9f, 0xd5, 0x20, 0x6e, 0x64, 0x4a, 0x1b, 0x50}, c.nameKey)
	assert.Equal(t, [16]uint8{0x3e, 0xa9, 0x5e, 0xf6, 0x81, 0x78, 0x2d, 0xc9, 0xd9, 0x95, 0x5d, 0x22, 0x5b, 0xfd, 0x44, 0x2c}, c.nameTweak)

	require.NoError(t, c.Key("potato", "Sausage"))
	assert.Equal(t, [32]uint8{0xda, 0x81, 0x8c, 0x67, 0xef, 0x11, 0xf, 0xc8, 0xd5, 0xc8, 0x62, 0x4b, 0x7f, 0xe2, 0x9e, 0x35, 0x35, 0xb0, 0x8d, 0x79, 0x84, 0x89, 0xac, 0xcb, 0xa0, 0xff, 0x2, 0x72, 0x3, 0x1a, 0x5e, 0x64}, c.nameKey)
	assert.Equal(t, [16]uint8{0x2, 0x81, 0x7e, 0x7b, 0xea, 0x99, 0x81, 0x5a, 0xd0, 0x2d, 0xb9, 0x64, 0x48, 0xb0, 0x28, 0x27}, c.nameTweak)
